...
export const fetchData = async ({ name, category, type, orientation, color }) => {
		...
		const API_URL = `https://pixabay.com/api/?key=${API_KEY}&q=${name}&image_type=photo&per_page=100&category=${category}&image_type=${type}&orientation=${orientation}&color=${color}$safesearch=true`;
